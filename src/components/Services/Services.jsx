...
const Services = () => {
  ...
  useEffect(() => {
    const fetchData = async () => {
      setLoading(true)
      try {
        // Fetch categories (only GET from /category)
        const categoriesRes = await fetch(`${API_URL}/category`)
        if (!categoriesRes.ok) throw new Error('Failed to fetch categories')
        const categoriesData = await categoriesRes.json()
        setCategories(categoriesData)

        // Fetch services (GET from /service)
  ...
  {services.map(service => (
    <tr key={service.id}>
      ...
      <td>{service.price.toLocaleString()} so'm</td> {/* Shu yer o'zgardi */}
      <td>
        <button
